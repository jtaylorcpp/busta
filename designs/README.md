...
Approved mockups (and parked ones, marked **To do**), saved as self-contained HTML (open `index.html` in a
...
| 2026-09-25 | [Sort existing mail](2026-09-25-sort-existing-mail/index.html) (v3) | "Test this rule" checks every email the save would sort (30 days / 200, Messages and every folder) with progress, matches and closest misses, or says none match; **Where to look** replaces "Sort recent mail": Messages and each folder with how many of its emails would go in; saving reuses the test; a folder's own mail is re-checked when its rule changes; "N moved here" with Undo | [artifact](https://claude.ai/artifact/CqKDVjAt6hfaTK71VbJZoi) | `web/src/components/islands/{RuleTester,SortPlaces}.tsx`, `web/src/components/app/FolderEditor.astro`, `web/src/pages/mb/[address]/folders/{test,save}.ts`, `folders/[folder]/unsort.ts`, `src/folders.ts` (`sortInto`), `MailboxDO` (sort candidates, undo) |
| 2026-09-25 | [Folder order](2026-09-25-folder-order/index.html) (v1) | Drag folders into order on the Folders page or in the sidebar (arrows and Alt+↑↓ as fallbacks); the order is the sort order: each folder's rule gets its own yes/no score and the first at 75% or more wins; new folders go to the bottom; the filing popover says why ("checked 2nd; School fits too, but it's checked 4th") | [artifact](https://claude.ai/artifact/JokN1FK61oSGpPz22LtJM2) | `src/classify.ts` (`classifyEach`), `src/folders.ts`, `MailboxDO.reorderFolders`, `web/src/pages/mb/[address]/folders/{index.astro,order.ts}`, `web/src/components/islands/FolderOrder.tsx`, `MailboxNav.astro`, `ThreadFolder.astro` |
| 2026-09-25 | **To do** · [Text Busta](2026-09-25-text-busta/index.html) (v7) | Texting for a dumb phone: phone verified with Twilio Verify, a PIN, and every feature opt-in with its risks (texts for folders you choose, sender and subject, one-time 12-hour PIN links, Ask by text); one-time email and answer pages; short small-screen answers from a jev-ranked pipeline over ThreadDOs; a no-browser mode; the threat model (22 threats, must-haves) | [artifact](https://claude.ai/artifact/TvGx9ydL8vpe3tywtnQkAm) | Built, not live: waiting on Twilio activation, then `TWILIO_FROM`, a deploy and an end-to-end test (Ask needs production). Code: `src/sms/*`, `src/{phone,link,ask}-do.ts`, `web/src/pages/{texting.astro,v,a}`, `web/src/lib/links.ts`, `ThreadDO.relevance/readForAnswer`, `TenantDO` (texting, questions) |
| 2026-09-25 | **To do** · [Busta for desktop](2026-09-25-desktop/index.html) (v2, not approved) | A desktop app for Linux, macOS and Windows: Electron running Busta's own Astro pages locally (Node build), mail kept in the cloud over Cap'n Web (`wss://busta.app/rpc`), connect this computer (browser approval, keychain token), your computers (disconnect), tray, notifications, reconnecting, security table, how it works and build order (data layer, actions into Astro, `/rpc`, two builds) | [artifact](https://claude.ai/artifact/SCLo1HoCT7guZvTGvXwnGs) | Not built |
...
- **Text Busta, as built:** the question pipeline runs in a Durable Object
  per question (AskDO, alarm-driven steps) rather than a Cloudflare Workflow.
  "Questions you texted" is a section of Texting settings, not its own page.
  The per-Ask email alert is not built yet; each question is logged there
  instead. The folder editor's "Text me" switch isn't built; folders are
  picked in Texting settings.
